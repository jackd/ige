...
class Skeleton(object):
    ...
    is interpretted as being a left or right joint respectively. This can be
    used for data augmentation. See `Skeleton.flip_left_right`.
    ...
    `('l_knee', 'l_hip')`, then associated arrays are assumed to have the
    relevant quantity about the left knee in index 1. This is helpful for
    plotting.
    """
    ...
    def __init__(self, links, name=None):
        ...
        ```
        would represent a disconnected left leg. The left knee would be
        associated with index `1`.

        Every `parent` should exists as a `child`

        Args:
            `links`: iterable of `(child, parent)` iterables. Joints are
                indexed by their position in `links` as the child.
        """
