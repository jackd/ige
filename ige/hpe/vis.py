...
def get_limb_data(skeleton, points, visible=None):
    for child in range(skeleton.num_joints):
...
def update_lines2d(skeleton, lines, points):
    for line, (data, _) in zip(lines, get_limb_data(skeleton, points)):
        line.set_xdata(data[:, 0])
        line.set_ydata(data[:, 1])


def vis2d(skeleton, points, ax=None, change_ax=None, kwargs_fn=None,
          visible=None, **constant_kwargs):
    """Visualize 2D skeleton data using matplotlib.pyplot."""
    if points.shape != (skeleton.num_joints, 2):
        raise ValueError('Expected points of shape %s, got %s'
                         % ((skeleton.num_joints, 2), points.shape))
    if change_ax is None:
        change_ax = ax is None
    if ax is None:
        ax = default_ax2d()
    if change_ax:
        ax.invert_yaxis()
...
def vis3d(skeleton, points, ax=None, change_ax=None, kwargs_fn=None,
          **constant_kwargs):
    """Visualize 3D skeleton data using matplotlib.pyplot."""
    from mpl_toolkits.mplot3d import Axes3D  # NOQA
    if change_ax is None:
        change_ax = ax is None
    if points.shape != (skeleton.num_joints, 3):
        raise ValueError('Expected points of shape %s, got %s'
                         % ((skeleton.num_joints, 3), points.shape))
